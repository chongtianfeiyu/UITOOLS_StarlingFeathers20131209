...
//			reset();
		...
		private function setFrame():void
		{
			...
			for(var i:int = 0;i<len;i++)
			{
				...
				if(imagevo.hasAlpha&&imagevo.alpha!=1)
				{
...
//				var image:Image  = new Image(mtextureAtlas.getTexture(imagevo.name))
				image.texture = mtextureAtlas.getTexture(imageNames[imagevo.name]);
				image.readjustSize();
...
//				image.width = image.texture.width;
			if(vo.hasAlpha)
				image.alpha =vo.alpha;// propertyTovalue("alpha",vo.alpha)
			else if(setdefault)
				image.alpha = 1;
			if(vo.hasPivotX)
				image.pivotX =vo.pivotX;// propertyTovalue("pivotX",vo.pivotX)
			else if(setdefault)
				image.pivotX = 0;
			if(vo.hasPivotY)
				image.pivotY =vo.pivotY;// propertyTovalue("pivotY",vo.pivotY)
			else if(setdefault)
				image.pivotY = 0;
			if(vo.hasScaleX)
				image.scaleX =vo.scaleX;// propertyTovalue("scaleX",vo.scaleX)
			else if(setdefault)
				image.scaleX = 1;
			if(vo.hasScaleY)
				image.scaleY =vo.scaleY;// propertyTovalue("scaleY",vo.scaleY)
			else if(setdefault)
			    image.scaleY =	1;
			if(vo.hasSkewX)
				image.skewX = vo.skewX* ANGLE_TO_RADIAN;//propertyTovalue("skewX",vo.skewX)
			else if(setdefault)
				image.skewX =0;
			if(vo.hasSkewY)
				image.skewY = vo.skewY* ANGLE_TO_RADIAN;//propertyTovalue("skewY",vo.skewY)
			else if(setdefault)
				image.skewY = 0;
//			if(vo.hasRotation)
...
//				image.color = propertyTovalue("color",vo.color)
			...
			if(vo.hasX)
				image.x =vo.x;// propertyTovalue("x",vo.x)
			if(vo.hasY)
				image.y =vo.y;// propertyTovalue("y",vo.y)
